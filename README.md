...
| Live AI log viewer | Debug tool: `Config` tab → **Open viewer** shows every prompt and response the backend exchanged with the AI, live, with timing, filters, search and copy buttons. Off unless `MT_LIVE_AI_LOG_ENABLED=true` on the backend; on a hosted backend only the `MT_ADMIN_EMAIL` account can open it. A **Save images** switch (off by default — it costs disk space and background work per call) also keeps the images sent to the AI so you can look at them, click to enlarge, and compare with the prompt. |
