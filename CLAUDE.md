...
- `MT_REQUIRE_AUTH` (default off) gates the backend behind account tokens; `MT_DATABASE_URL` points it at a real Postgres for those accounts; `MT_ADMIN_EMAIL` names the one account allowed to manage the Owner shared LLM config — see "Hosted/account mode" above. Irrelevant to the normal local setup.
