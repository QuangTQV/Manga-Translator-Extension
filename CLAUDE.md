...
- **Live AI log** (`core/live_ai_log.py`, off unless `MT_LIVE_AI_LOG_ENABLED`): logs every LLM call from the single choke point `_call_llm_endpoint_impl` (text only, no image bytes); `call_type` travels via a `ContextVar`, not a parameter, because tests patch that function with fixed-arity mocks. Optional image capture (`MT_LIVE_AI_LOG_IMAGES`, or the viewer's runtime **Save images** switch via `GET/POST /admin/live-ai-log/settings`; per process, resets on restart): the images sent to the model are written as content-hash-named files under `logs/live_ai_images/` (de-duplicated, pruned to `MT_LIVE_AI_IMAGES_MAX_MB`) and referenced from the entry (`images: [{id, mime, kb}]`), served by `GET /admin/live-ai-log/images/{id}` (id validated as 32 lowercase hex — never a path). **Keep it off the request path:** the decode/write/append all happen on one background thread (`live_ai_log._finish_entry`), the entry line is appended only after its images are on disk, and while anything is queued even non-image calls go through the same thread so timestamps stay in order (the viewer's `since` polling relies on that). When the switch is off nothing is queued at all. Viewed via `GET /admin/live-ai-log` (`limit`, and `since` = only entries newer than that timestamp; the file is read backwards in chunks so polling a 20 MB log stays cheap) — and in the extension by the `extension/src/live-ai/` page, a separate Vite HTML entry (`vite.config.ts` moves every page in its list to `dist/<page>/`) opened from the popup's Config tab. The page asks the service worker (`LIVE_AI_LOG` message → `fetchLiveAiLog()`, which attaches the account token) and renders log text with `textContent` only — it holds real prompts and model output.
...
**Hover/click targets:** every time `renderPage()` redraws a page it calls `deps.onRegionsChanged`, and the content script turns each drawn region (not `restoreOnly` ones, not empty translations) into a hit target on the same `.mt-fix-hit` layer detected bubbles use — hover = magnified crop with the region's original text, click = `editManualRegion()` (opens the editor on that exact region id). `renderBubbleFixTargets(img, detectedBubbles)` appends them AFTER the detected bubbles (a fix-hint sends back a bubble's index, so detected ones must stay first) and re-adds them on every render, so an auto-translation landing can't wipe them.
