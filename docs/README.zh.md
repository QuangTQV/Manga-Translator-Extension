...
<img src="assets/auto-translate-preview.png" alt="正在自动翻译的漫画阅读页面：第一页已翻译，一个气泡被放大并显示日文原文，下一页仍在翻译中" width="560">

在弹窗中点击 **自动翻译**（或按 **Alt+Shift+A**，Mac 上为 **⌘+Shift+A**），然后照常阅读即可。页面在进入视野时被翻译，并会提前翻译后面几页，因此通常在你滚动到下一页之前它就已经翻译好了。

- **蓝色 `•••` 标记**：该页正在翻译（上图下方的页面）。
- **绿色 `MT` 标记**：该页已完成。连续失败 3 次的页面会显示红色标记，点击即可重试。
- **将鼠标悬停在气泡上**可放大查看，下方附有原文以便核对译文。每页都有一个按钮可在译文和原图之间切换。
- 角落里的 **自动 MT** 标签会统计已翻译的页数，点击 **停止** 结束本次自动翻译。

开启 **提前翻译** 后，页面一加载就开始翻译，而不是等你滚动到附近（阅读更流畅，但会消耗更多 API 调用）。弹窗顶部的 **启用扩展** 开关可以关闭全部功能，确保不会意外发送任何请求。

### 故事数据库与帮助聊天

| 故事数据库 | 帮助聊天 |
| --- | --- |
| <img src="assets/story-db-preview.png" alt="带关系图和角色列表的故事数据库标签页" width="390"> | <img src="assets/help-chat-preview.png" alt="帮助聊天回答如何获得更好的翻译" width="390"> |
| 把一个故事的角色、人物关系和固定译名放在一处，让名字和称呼在各章节之间保持一致。在关系图中拖动角色，或用“连线”把两个角色连起来。 | 在弹窗中点击 **?**，询问如何设置某项功能。回答来自你自己的 LLM，并以本项目的文档为依据。 |

### 修补自动翻译遗漏的地方

<img src="assets/manual-region-preview.png" alt="手动文字区域工具翻译一个未翻译的拟声词" width="560">

自动翻译漏掉了拟声词“ピンポーン”。点击 **✂ 框选文字区域**，在它上面拖出一个框，文字会被自动识别。自己输入译文或点击 **用 AI 翻译**，再点 **应用**，该处会被清除并绘入新文字。对于矩形框无法干净隔离的弯曲或倾斜文字，请改用 **🩹 橡皮擦** 涂抹。
...
| 自动翻译 | 点击 **自动翻译**（或按 **Alt+Shift+A**，Mac 上为 **⌘+Shift+A**）后照常阅读：页面在进入视野时被翻译，并提前翻译后面几页。浮动的 **自动 MT** 标签统计已翻译页数，并带有 **停止** 按钮。可选的 **提前翻译** 会在页面一加载就开始翻译。 |
...
| 故事数据库（可选，需登录） | 每个故事独立的角色数据库——角色（姓名/性别/角色定位/语气，另可选头像和最多 2 张参考图）、人物关系、固定译名术语表（每条术语可设为**强制精确**：翻译后把术语本身的写法及你列出的变体自动改写为确切译文——只能修正 AI 实际写出的内容，无法修正你未列出的其他译法），以及剧情延续笔记——与账户同步。**只有在 `Translate` 标签页开启 `使用故事数据库` 开关（默认关闭）时才会应用到翻译**——仅在 `Story DB` 标签页选中某个故事并不够。交互式**关系图**（拖动角色、点击高亮、用”连线”为两个角色添加关系）让你一眼看清角色阵容，布局会被保存。参考图仅在你开启”向 AI 发送参考图”时才会发送给模型。未保存的修改在关闭弹窗后依然保留——下次会自动恢复，也可以点击丢弃从头开始。支持 Undo/Redo（Ctrl+Z / Ctrl+Shift+Z）在修改之间前后切换。由于当前选中的故事是所有网站共用的一项设置（不是按页面区分），如果当前网站上次翻译使用的故事与现在选中的不同，会出现提示横幅——不会自动切换任何内容，只是提醒你确认一下。可在 `Story DB` 标签页管理。 |
...
| 重新翻译整页 | 每个已翻译的页面在其 MT / 下载 / 查看原图按钮下方（页面右上角）都有一个 **↻** 按钮。点击后会用你*当前*的设置（模型、语言、Story DB、指令……）重新翻译这一页——新译文到达前旧译文一直保留，失败时不会有任何改变，并且即使后端有缓存也一定会重新请求模型。用 click-to-fix 做的修正会丢失；手动文字区域会重新应用。 |
| 移动/删除气泡 | 检测框位置不对，或本不该有气泡？打开该气泡的 Fix 弹窗，点击 **Move**（拖出新框；旧位置会恢复为原图）或 **Delete**（只恢复原图）。 |
| 手动文字区域 | 对某处不满意，或自动检测漏掉了？在弹窗点击 **✂ 框选文字区域**，在页面任意文字上拖出一个框：文字会被自动识别（OCR），然后你可以自己输入译文，或点击 **用 AI 翻译**（会使用 Story DB 和你的指令）。该处会被清除并把译文绘回图片。再次框选同一位置可编辑或删除；区域会按页面记住。应用后，该文字区域的行为与检测到的气泡一样：悬停可查看带原文的放大图，点击可编辑。需要运行最新版 backend。 |
| 橡皮擦 | 对于矩形无法干净隔离的原文/SFX（弯曲或倾斜的 SFX、贴着角色轮廓的文字）：点击 **🩹 橡皮擦** 然后用画笔涂抹，该处会被修复清除。与手动文字区域工具一样按页面记住。 |
...
| CBZ / PDF 导出 | ZIP 导出按钮旁的 "Export CBZ" 和 "Export PDF"——同样的页面，但按扫描顺序编号，方便正确翻页（普通 ZIP 导出的文件名来自源 URL，不一定符合阅读顺序）。 |
...
| Live AI 查看器 | 调试工具：`Config` 标签页 → **Open viewer**，实时显示后端与 AI 之间的每个提示词和响应，附带耗时、筛选、搜索和复制按钮。仅当后端设置 `MT_LIVE_AI_LOG_ENABLED=true` 时可用；在托管后端上只有 `MT_ADMIN_EMAIL` 账户可以打开。 |
...
| 文字识别方式 | `Translate` 标签页的选项：让 AI 读取每个气泡图片（默认），或先用 manga-ocr / PaddleOCR-VL 在本机识别文字，只把文本发给 AI——更省钱、可配合纯文本模型，并让“翻译之前”词典精确生效。 |
| 本地 LLM | 通过 `OpenAI-Compatible` provider 使用 Ollama 或 LM Studio（视觉模型，或搭配本地 OCR 的任意文本模型）完全离线翻译，无 API 费用——见 [本地 LLM](#本地-llmollama--lm-studio)。 |
...
| Tab | 选项 |
| --- | --- |
| `Translate` | 源语言、目标语言、`使用故事数据库` 开关（默认关闭——需开启后 Story DB 标签页的数据才会实际影响翻译）、气泡外文字开关、Previous-page context、上下文记忆、故事笔记（可点击"Suggest"生成草稿）。 |
...
**默认需要视觉模型。** AI 直接从页面图片读取文字，纯文本模型（普通 Llama、Qwen 2.5 等）看不到任何可翻译的内容。可识图的模型示例：Qwen2.5-VL（Ollama 中为 `qwen2.5vl:7b`）、Gemma 3（`gemma3:12b`），或 LM Studio 标记为视觉能力的任意模型。

**纯文本模型也能用**：把 `Translate` 标签页的 **文字识别方式** 设为 `manga-ocr`（仅日语）或 `PaddleOCR-VL`，文字会先在本机识别，只有文本发给模型。这样对本地 GPU 也更省、更轻，代价是 AI 看不到气泡图片。
...
**Q: 我设置并选中了故事数据库，但翻译好像没有用上它，为什么？**

A: 请开启 `Translate` 标签页的 `使用故事数据库` 开关——仅在 `Story DB` 标签页选中故事并不会自动应用，这是有意如此设计的，让你即使选中了某个故事，也能翻译不需要它的单页内容。

**Q: 我不太清楚某个设置该怎么弄，扩展里有内置帮助吗？**

A: 点击弹窗顶部的 **?** 按钮直接提问。回答由你自己配置的 LLM 生成，依据本项目自身的文档，而不是预设脚本——所以会消耗你一点 API 额度，和这里的其他 AI 功能一样。
