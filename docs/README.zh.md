...
| 故事数据库（可选，需登录） | 每个故事独立的角色数据库——角色（姓名/性别/角色定位/语气，另可选头像和最多 2 张参考图）、人物关系、固定译名术语表，以及剧情延续笔记——与账户同步，翻译该故事时自动使用。交互式**关系图**（拖动角色、点击高亮、用“连线”为两个角色添加关系）让你一眼看清角色阵容，布局会被保存。参考图仅在你开启“向 AI 发送参考图”时才会发送给模型。未保存的修改在关闭弹窗后依然保留——下次会自动恢复，也可以点击丢弃从头开始。可在 `Story DB` 标签页管理。 |
