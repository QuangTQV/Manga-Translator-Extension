...
| CSDL truyện (tùy chọn, cần đăng nhập) | Cơ sở dữ liệu nhân vật riêng cho từng truyện — nhân vật (tên/giới tính/vai trò/giọng điệu, kèm ảnh đại diện và tối đa 2 ảnh tham chiếu tùy chọn), mối quan hệ, thuật ngữ dịch cố định, và ghi chú diễn biến — đồng bộ theo tài khoản, dùng tự động khi dịch đúng truyện đó. **Sơ đồ quan hệ** tương tác (kéo nhân vật, bấm để làm nổi bật, nút "Nối" để thêm quan hệ giữa hai nhân vật) giúp nhìn cả dàn nhân vật, vị trí được lưu lại. Ảnh tham chiếu chỉ gửi cho AI khi bạn bật "Gửi ảnh tham chiếu cho AI". Quản lý ở tab `Story DB`. |
...
| Cài đặt font | Chọn font pack vẽ chữ đã dịch, và khoảng cỡ chữ nhỏ nhất/lớn nhất, ngay trong tab Translate — bỏ font pack của bạn (thư mục chứa file .ttf/.otf) vào `backend/fonts/` để thấy trong danh sách. |
