...
| Dịch lại một trang | Mỗi trang đã dịch có nút **↻** dưới các nút MT / tải xuống / xem bản gốc (góc phải trên của trang). Bấm để dịch lại đúng trang đó với cài đặt *hiện tại* (model, ngôn ngữ, Story DB, chỉ dẫn...) — bản dịch cũ vẫn hiện cho tới khi có bản mới, nếu lỗi thì không có gì thay đổi, và luôn hỏi lại model kể cả khi backend đang có sẵn bản dịch trong cache. Các chỉnh sửa bằng click-to-fix sẽ mất; vùng chữ vẽ tay được áp lại. |
...
| Vùng chữ thủ công | Chưa ưng một chỗ, hoặc auto-detect bỏ sót? Bấm **✂ Chọn vùng chữ** trong popup rồi kéo khung quanh chữ bất kỳ trên trang: chữ được đọc tự động (OCR), sau đó bạn tự gõ bản dịch hoặc bấm **Dịch bằng AI** (áp dụng Story DB và chỉ dẫn của bạn). Chỗ đó được xóa chữ và vẽ lại bản dịch vào ảnh. Chọn lại đúng chỗ để sửa hoặc xóa; các khung được nhớ theo từng trang. Sau khi áp dụng, vùng chữ hoạt động như một bong bóng được phát hiện: trỏ vào để xem bản phóng to kèm chữ gốc, bấm vào để sửa. Cần chạy backend bản mới nhất. |
...
| Trình xem Live AI | Công cụ debug: tab `Config` → **Open viewer** hiện mọi prompt và phản hồi giữa backend và AI theo thời gian thực, kèm thời gian xử lý, bộ lọc, tìm kiếm và nút sao chép. Chỉ hoạt động khi backend đặt `MT_LIVE_AI_LOG_ENABLED=true`; trên backend hosted chỉ tài khoản `MT_ADMIN_EMAIL` mới mở được. Có thêm công tắc **Save images** (mặc định tắt — tốn dung lượng và xử lý nền ở mỗi lệnh gọi) để giữ lại các ảnh gửi cho AI, xem, bấm phóng to và đối chiếu với prompt. |
