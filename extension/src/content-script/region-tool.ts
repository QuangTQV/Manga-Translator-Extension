...
async function renderPage(img: HTMLImageElement, rawUrl: string, regions: StoredRegion[]): Promise<void> {
  ...
  const out = await api<{ image: string }>('/region/render', {
    image: base,
    regions: regions.map((r) => ({ box: r.box, text: r.translation })),
  });
...
export function startRegionSelect(): void {
  ...
  hint.textContent = deps.tr('regionPickHint');
...
async function openEditor(img: HTMLImageElement, rawUrl: string, sel: Rect): Promise<void> {
  closeEditor?.();
  const box = toNormBox(sel, img);
  const all = await regionsFor(rawUrl);
  const existing = overlapsExisting(box, all);
  const region: StoredRegion = existing ?? { id: crypto.randomUUID(), box, text: '', translation: '' };
  ...
  applyBtn.addEventListener('click', async () => {
    ...
    try {
      await commit([...others, { ...region, text: orig.value.trim(), translation: trans.value.trim() }]);
      close();
    } catch (e) {
      setStatus(e instanceof Error ? e.message : String(e), true);
      setBusy(false);
  ...
  // A brand-new region: read its text right away.
  if (!existing) {
    setBusy(true);
    orig.disabled = true;
